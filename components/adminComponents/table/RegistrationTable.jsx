...
export function RegistrationTable({ data: initialData, userRole }) {
  ...
  const [selectedEventTransId, setSelectedTransId] = React.useState(null);

  const handleDownloadClick = (event) => {
    setSelectedEvent(event);
    setShowModal(true);
  };
  const handleDownloadInvoice = (transId) => {
    setSelectedTransId(transId);
    setInvoiceShowModal(true);
  };

  const columns = React.useMemo(
    () => getColumns(userRole, handleDownloadClick, handleDownloadInvoice),
    [userRole]
  );
  ...
  return (
    <>
      ...
      {selectedEventTransId && (
        <DownloadInvoiceModal
          isOpen={invoiceShowModal}
          onClose={() => setInvoiceShowModal(false)}
          transId={selectedEventTransId}
        />
      )}
    </>
  );
